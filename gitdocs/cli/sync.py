    ...
    try:
        ...
        try:
            ...
            for ticket_key, ticket_commit_list in ticket_commits.items():
                commit_summaries = "\n".join(
                    [f"- {c.sha[:7]}: {c.message.split(chr(10))[0]}" for c in ticket_commit_list]
                )
        ...
        if suggestions:
            for s in suggestions:
                conf_raw = s["confidence"]
                conf: float = float(conf_raw) if isinstance(conf_raw, int | float) else 0.0
                confidence_color = "green" if conf >= 0.7 else "yellow"
                console.print(f"\n[bold cyan]{s['ticket']}[/] ({s['commits']} commits)")
                console.print(f"Confidence: [{confidence_color}]{conf:.0%}[/]")
    ...
    try:
        ...
        if not comment_text:
            ...
            try:
                ...
                if relevant:
                    commits_str = "\n".join(
                        [f"- {c.sha[:7]}: {c.message.split(chr(10))[0]}" for c in relevant]
                    )
