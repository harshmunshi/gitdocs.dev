...
class MappingStore:
    ...
    def from_dict(cls, data: dict) -> "MappingStore":
        ...
        store = cls()
        for _ticket, mappings in data.items():
            for m in mappings:
                store.add_mapping(
                    TicketCommitMapping(
                        ticket_key=m["ticket_key"],
                        ...
                        synced_at=(
                            datetime.fromisoformat(m["synced_at"]) if m.get("synced_at") else None
                        ),
                    )
                )
        return store
    ...
    for pattern in patterns:
        try:
            ...
            for match in matches:
                ...
                key = match[0].upper() if isinstance(match, tuple) else match.upper()
                keys.add(key)
